...
class Scene_q9(VoiceoverScene):
    def construct(self):
        ...
        # results rail (fixed slots, no re-stacking)
        self._ri = 0
        def add_result(txt, color=SIGNAL):
            c = chip(txt, color=color, tscale=0.30).move_to(RIGHT * RAIL_X + UP * RAIL_Y[self._ri])
            self._ri += 1
        ...
        qline = VGroup(
            ...
            Mono("(45°,45°) → hit after T1     (60°,30°) → hit after T2", color=SIGNAL).scale(0.42),
        ).arrange(DOWN, buff=0.38).next_to(head, DOWN, buff=0.7)
        target = Label("Find  (T1 / T2)²", color=IGNITION).scale(0.68).next_to(qline, DOWN, buff=0.6)
        ...
        g1 = Mono("GIVEN:  v0, θ0, θ1, gap L", color=SIGNAL).scale(0.55)
        g2 = Body("trigger:  \"same instant\"  ·  \"suitable speed to hit\"", color=AMBER).scale(0.46)
        g3 = Label("target:  (T1 / T2)²  — a pure number", color=IGNITION).scale(0.5)
        ...
        with self.voiceover(text="That fixes the stone's suitable speed — and means they never differ in height.") as t:
            ...
            add_result("v₁ = v₀sinθ₀ / sinθ₁", IGNITION)
        ...
        with self.voiceover(text="Substitute and tidy up, and it folds into a clean form: L sine theta-one over "
            ...
            add_result("T = Lsinθ₁ / v₀sin(θ₀+θ₁)", SIGNAL)
        ...
        # end card — like & subscribe
        big = on_logo(0.7).move_to(UP * 1.6)
        sub = Label("LIKE  &  SUBSCRIBE", color=IGNITION).scale(0.8).move_to(DOWN * 0.2)
        cta = Body("for more JEE Main & Advanced solutions", color=TITANIUM).scale(0.5).next_to(sub, DOWN, buff=0.4)
        mantra = Mono("In pursuit of mastery.", color=EMBER).scale(0.4).next_to(cta, DOWN, buff=0.7)
        with self.voiceover(text="If this made it click, like the video and subscribe — we solve one JEE problem, "
                                 "completely, every time. In pursuit of mastery.") as t:
            self.play(FadeIn(big, shift=DOWN * 0.15), run_time=1.0)
            self.play(Write(sub), run_time=0.9)
            self.play(FadeIn(cta, shift=UP * 0.1), run_time=0.6)
            self.play(FadeIn(mantra), run_time=0.6)
            self.wait(max(0.4, t.duration - 3.1))
        self.wait(0.7)
